...
import { HandleMenuTypeDelete } from "../../../../hooks/menus/Forms";
...
// Modal para eliminar los menús de la tabla
export default function Menu_Delete(){
    ...
    useEffect(() => {
        if(isMenuTypeDelete){
            const promise = new Promise((resolve,reject) => {
                try{
                    setTimeout(() => {
                        socket.emit('Insert-Deleted-Menu-Type',isLoggedUser.idusuario,isTextFieldsMenuType.idtipo)

                        resolve('¡Eliminó al menú!');

                        setIsMenuTypeDelete(false);
            ...
            {isModal && isSelectedRow !== null ? (
                <>
                    <Container_Modal_Background_Black ref={Modal}>
                        <Image_Modal/>
                        <Container_Modal_Form_White_500 ref={isForm} className={currentMView === 'Tipo-Menu-Eliminar' ? 'slide-in-container-top' : 'slide-out-container-top'}>
                            <Container_Modal_Form_White>
                                <Container_Modal_Form>
                                    <Text_Title_28_Black>ELIMINAR MENÚ</Text_Title_28_Black>
                                    <Form_Verification/>
                ...
                currentMView === 'Tipo-Menu-Eliminar' ? (
                    <>
                        <Error_Delete/>
                    </>
                ):(
