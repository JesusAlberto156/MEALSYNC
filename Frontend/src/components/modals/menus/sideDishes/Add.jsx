...
import { SideDishAddContext } from "../../../../contexts/SideDishesProvider";
...
import { HandleSideDishAdd } from "../../../../hooks/sideDishes/Forms";
...
// Modal para agregar una guarnición
export default function Side_Dish_Add(){
    ...
    const handleSideDishAdd = HandleSideDishAdd();
    ...
    useEffect(() => {
        if(isSideDishAdd){
            const promise = new Promise((resolve,reject) => {
                try{
                    setTimeout(() => {
                        socket.emit('Insert-Side-Dish',isLoggedUser.idusuario,isTextFieldsSideDish.nombre.trim(),isTextFieldsSideDish.idmenu,isTextFieldsSideDish.descripcion.trim(),isTextFieldsSideDish.precio,isTextFieldsSideDish.preparacion,isTextFieldsSideDish.imagen,isTextFieldsSideDish.tipos,isTextFieldsSideDish.ingredientes);

                        resolve('¡Agregó la guarnición!');

                        setIsSideDishAdd(false);
            ...
            return Alert_Sonner_Promise(promise,'¡Agregando la guarnición!','2');
        }
    },[isSideDishAdd]);
            ...
            {isModal ? (
                <>
                    <Container_Modal_Background_Black>
                        <Image_Modal/>
                        <Container_Modal_Form_White_600 className={currentMView === 'Guarnicion-Agregar' ? 'slide-in-container-top' : 'slide-out-container-top'}>
                            <Container_Modal_Form_White>
                                <Container_Modal_Form>
                                    <Text_Title_28_Black>AGREGAR GUARNICIÓN</Text_Title_28_Black>
                                    ...
                                    {isTextFieldsSideDish.tipos.length !== 0 ? (
                                        isTextFieldsSideDish.tipos.map((menu,index) => {
                                            const filteredRecordsMenuTypes = isMenuTypes.filter((data) => {
                                                ...
                                                if(isTextFieldsSideDish.tipos.some((ing, i) => i !== index && ing.idtipo === data.idtipo)) return false
