...
import { CleaningCategoryCountAddContext,CleaningCategoriesContext,DeletedCleaningCategoriesContext } from "../../../../contexts/ExtrasProvider";
...
import { HandleCountCleaningCategoryAdd } from "../../../../hooks/extras/Forms";
...
// Modal para agregar cantidades a las categorías de limpieza a su tabla
export default function Count_Cleaning_Category_Add(){
    ...
    const handleCountCleaningCategoryAdd = HandleCountCleaningCategoryAdd();
    ...
    useEffect(() => {
        setIsTextFieldsCleaningCategory(prev => ({...prev, cantidades: [{ cantidad: '' }]}))
    },[])
    ...
    useEffect(() => {
        if(isCleaningCategoryCountAdd){
            const promise = new Promise((resolve,reject) => {
                try{
                    setTimeout(() => {
                        socket.emit('Insert-Count-Cleaning-Category',isLoggedUser.idusuario,Number(isTextFieldsCleaningCategory.cantidades[0].cantidad),isTextFieldsCleaningCategory.idcategoria);

                        resolve('¡Agregó una cantidad a la categoría de limpieza!');

                        setIsCleaningCategoryCountAdd(false)
            ...
            return Alert_Sonner_Promise(promise,'¡Agregando una cantidad a la categoría de limpieza!','2');
        }
    },[isCleaningCategoryCountAdd]);
        ...
        <>
            {isModal && isSelectedRow !== null ? (
                <Container_Modal_Background_Black ref={Modal}>
                    <Image_Modal/>
                    <Container_Modal_Form_White_600 ref={isForm} className={currentMView === 'Categoria-Limpieza-Cantidad-Agregar' ? 'slide-in-container-top' : 'slide-out-container-top'}>
                        <Container_Modal_Form_White>
                            <Container_Modal_Form>
                                <Text_Title_28_Black>AGREGAR CANTIDAD A LA CATEGORÍA DE LIMPIEZA</Text_Title_28_Black>
                                ...
                                <Container_Row_100_Left>
                                    ...
                                    <Input_Group>
                                        <Input_Text_100_Black
                                            ...
                                            value={isTextFieldsCleaningCategory.cantidades?.[0]?.cantidad ?? ''}
                                            onChange={(e) => {
                                                if(!isNaN(Number(e.target.value))){
                                                    setIsTextFieldsCleaningCategory(prev => ({...prev, cantidades: [{ cantidad: e.target.value }]}))
                                                }
                                            }}
                                            onFocus={() => {
                                                if(isKeyboardTouch.current){
                                                    setIsKeyboard(true);
                                                    setIsKeyboardView('Cantidad-Categoria-Limpieza');
                                                }
                                            }}
                                        />
                                    </Input_Group>
                                ...
                                <Modal_Form_Button_Add
                                    ...
                                    onAction={() => handleCountCleaningCategoryAdd()}
                                    icon={<MdAddBox/>}
                                />
                            </Container_Modal_Form>
                        </Container_Modal_Form_White>
                ...
                currentMView === 'Categoria-Limpieza-Cantidad-Agregar' ? (
                    <>
                        <Error_Add/>
                    </>
                ):(
