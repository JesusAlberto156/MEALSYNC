...
import { CleaningSupplyDeleteContext,DeletedCleaningSuppliesContext } from "../../../../contexts/ExtrasProvider";
...
import { HandleCleaningSupplyDelete } from "../../../../hooks/extras/Forms";
...
// Modal para eliminar el suministro de limpieza de la tabla
export default function Cleaning_Supply_Delete(){
    ...
    const handleCleaningSupplyDelete = HandleCleaningSupplyDelete();
    ...
    useEffect(() => {
        if(isCleaningSupplyDelete){
            const promise = new Promise((resolve,reject) => {
                try{
                    setTimeout(() => {
                        socket.emit('Insert-Deleted-Cleaning-Supply',isLoggedUser.idusuario,isTextFieldsCleaningSupply.idsuministro)

                        resolve('¡Eliminó al suministro de limpieza!');

                        setIsCleaningSupplyDelete(false);
            ...
            return Alert_Sonner_Promise(promise,'¡Eliminando un suministro de limpieza!','1');
        }
    },[isCleaningSupplyDelete]);
            ...
            {isModal && isSelectedRow !== null ? (
                <>
                    <Container_Modal_Background_Black ref={Modal}>
                        <Image_Modal/>
                        <Container_Modal_Form_White_500 ref={isForm} className={currentMView === 'Suministro-Limpieza-Eliminar' ? 'slide-in-container-top' : 'slide-out-container-top'}>
                            <Container_Modal_Form_White>
                                <Container_Modal_Form>
                                    <Text_Title_28_Black>ELIMINAR SUMINISTRO DE LIMPIEZA</Text_Title_28_Black>
                                    <Form_Verification/>
                                    ...
                                    <Text_Span_12_Justify_Black>La eliminación de este suministro de limpieza impedirá agregar nuevos pedidos de inventario a este suministro de limpieza.</Text_Span_12_Justify_Black>
                                    <Modal_Form_Button_Delete
                                        onCancel={() => handleModalViewExtras('')}
                                        onAction={() => handleCleaningSupplyDelete()}
                                    />
                                </Container_Modal_Form>
                            </Container_Modal_Form_White>
                ...
                currentMView === 'Suministro-Limpieza-Eliminar' ? (
                    <>
                        <Error_Delete/>
                    </>
                ):(
