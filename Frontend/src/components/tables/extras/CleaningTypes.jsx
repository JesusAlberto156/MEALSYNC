...
import { CountCleaningTypesContext,CleaningCategoriesContext } from "../../../contexts/ExtrasProvider"
// Hooks personalizados
import { ResetTextFieldsUser } from "../../../hooks/users/Texts"
import { ResetTextFieldsCleaningType } from "../../../hooks/extras/Texts"
import { TableActionsCleaningTypes } from "../../../hooks/extras/Tables"
...
// Tabla de los tipos de limpieza
export default function Table_Cleaning_Types(){
    ...
    useEffect(() => {
        const handleClickOutside = (event) => {
            const table = document.getElementById("Table-Cleaning-Types");
    ...
    useEffect(() => {
        if(isSelectedRow !== null){
            const cantidadesFiltradas = isCountCleaningTypes.filter(count => count.idtipo === isSelectedRow.idtipo).map(count => ({ cantidad: count.cantidad}))
            ...
            resetTextFieldsCleaningType();
            resetTextFieldsUser();
        }
    },[isSelectedRow])
    // Constantes con la funcionalidad de los hooks
    const { handleRowClick,nextPageCleaningTypes,prevPage,currentRecordsCleaningTypes,currentPage,totalPagesCleaningTypes } = TableActionsCleaningTypes();
    const resetTextFieldsCleaningType = ResetTextFieldsCleaningType();
    const resetTextFieldsUser = ResetTextFieldsUser();
    // Estructura del componente
    return(
        <>
            <Table_Container>
                <Table id="Table-Cleaning-Types">
                    ...
                    <Table_Body_Tbody_White>
                        {currentRecordsCleaningTypes.map((type) => (
                            <tr 
                                key={type.idtipo}
                            ...
                            >
                                ...
                                <Table_Body_Td style={{ color: isSelectedRow === type ? 'white' : ''}}>{isCleaningCategories.find(category => category.idcategoria === type.idcategoria)?.nombre || 'Desconocida'}</Table_Body_Td>
            ...
            <Table_Pagination
                ...
                currentRecords={currentRecordsCleaningTypes}
                totalPage={totalPagesCleaningTypes}
                onNextPage={() => nextPageCleaningTypes()}
                onPrevPage={() => prevPage()}
            />
        </>
    );
