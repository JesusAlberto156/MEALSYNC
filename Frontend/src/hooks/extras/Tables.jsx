...
import { CleaningCategoriesContext,DeletedCleaningCategoriesContext,CountCleaningCategoriesContext,FixedExpensesContext,DeletedFixedExpensesContext,CleaningSuppliesContext,DeletedCleaningSuppliesContext } from "../../contexts/ExtrasProvider";
    ...
    const filteredRecordsCleaningSupplies = useMemo(() => {
        const filtered = isCleaningSupplies.filter((data) => {
            ...
            if (isSelectedOptionSearch === 'General') {
                ...
                const count = isCountCleaningCategories.find(count => count.idcantidad === data.idcantidad)?.cantidad;
        ...
        return [...filtered].sort((a, b) => {
            ...
            if(isSelectedOptionOrder === 'Cantidad'){
                return isSelectedOptionOrderDirection === 'Asc'
                ? isCountCleaningCategories.find(count => count.idcantidad === a.idcantidad)?.cantidad - isCountCleaningCategories.find(count => count.idcantidad === b.idcantidad)?.cantidad
                : isCountCleaningCategories.find(count => count.idcantidad === b.idcantidad)?.cantidad - isCountCleaningCategories.find(count => count.idcantidad === a.idcantidad)?.cantidad
            }

            return 0
        });
    }, [isCleaningSupplies, isDeletedCleaningSupplies, isCleaningCategories, isCountCleaningCategories, isSuppliers, isSearchTerm, isSelectedOptionSearch, isSelectedOptionOrderDirection, isSelectedOptionOrder]);
    // Total de registros visibles de la tabla
    const recordsPerPage = 6;
    // Indices de los registros
    const indexOfLastRecord = currentPage * recordsPerPage;
